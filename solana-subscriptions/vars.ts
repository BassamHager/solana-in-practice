...
if (!SECOND_WALLET) {
  throw new Error("SECOND_WALLET is not defined");
}

const SECOND_WALLET_KEYPAIR = Keypair.fromSecretKey(
  new Uint8Array(KEYPAIR_SECRET)
);
if (!SECOND_WALLET_KEYPAIR) {
  throw new Error("SECOND_WALLET_KEYPAIR is not available");
}
